...
const CanvasMouseHandler = (props) => {
    const { canvasRef, ticksScrollRef, pixelsPerBeatRef, selectionRef, selectedItemsRef, ghostItemRef, } = props;
    const { setActiveEditor } = (0, DmxMidiContext_1.useDmxMidiContext)();
    const { sendCurrentTickToServer } = (0, RealTimeContext_1.useRealTimeContext)();
    // Keep all non-ref props fresh so the registered-once handlers never use stale closures
    const p = (0, react_1.useRef)({
        ...props,
        isItemInSelection: props.isItemInSelection ?? ((item, selected) => selected.includes(item)),
        x0: props.x0 ?? 0,
        editorMode: props.editorMode ?? 'TrackEditor',
        setActiveEditor,
    ...
    const onMouseUp = (_event) => {
        if (selectionRef.current?.mode == 'drag') {
            ...
            p.current.updateSelectedItems(selectedItemsRef.current.map(i => p.current.transformItem(i, deltaX, deltaY)));
        }
    ...
    const setSelectedItems = (items) => {
        ...
        if (p.current.onSelectedItemsChange)
            p.current.onSelectedItemsChange();
    };
    const onMouseDown = (event) => {
        ...
        if (event.clientY - canvasTop() >= 0 &&
            ...
            p.current.sendCurrentTickToServer((0, utils_1.xToTicks)({
    ...
    const onMouseMove = (event) => (!selectionRef.current ? onMouseUpMove : onMouseDownMove)(event);
    const onMouseUpMove = (event) => {
        if (p.current.itemsInRect({
            ...
            ghostItemRef.current = p.current.itemFromXY(event.clientX - canvasLeft(), event.clientY - canvasTop());
        }
    };
    const onWheel = (e) => {
        e.preventDefault();
        ...
        if (Math.abs(e.deltaX) > Math.abs(e.deltaY) && e.deltaX != 0) {
            ...
            const tickAtCursor = ticksScrollRef.current + (cursorX - p.current.x0) * utils_1.PPQ / oldPixelsPerBeat;
            pixelsPerBeatRef.current = newPixelsPerBeat;
            ticksScrollRef.current = Math.max(0, tickAtCursor - (cursorX - p.current.x0) * utils_1.PPQ / newPixelsPerBeat);
        }
    };
    (0, react_1.useEffect)(() => {
...
exports.default = CanvasMouseHandler;
//# sourceMappingURL=CanvasMouseHandler.js.map